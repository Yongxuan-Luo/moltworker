...
## 输出语言 / Output Language（新增）

从用户输入中解析可选头部（推荐写在开头）：
- `lang: zh|en|auto`
- `bilingual: true|false`
- `platform: webnovel|wattpad|medium|reddit|other`
- `glossary: true|false`

规则：
- `lang: en`：全程用英文输出（地道写作/分析，不要逐句翻译腔）
- `lang: zh`：全程用中文输出（自然中文，不要英译腔）
- `lang: auto`/未填写：跟随用户输入的主语言
- `bilingual: true`：先主语言后译文；专有名词保持一致（按用户偏好：保留/音译/意译）
- 如果素材语言与输出语言不同：做**本地化**（localization），保留事实与人物动机，但调整习语、比喻、对话节奏以适配目标读者
